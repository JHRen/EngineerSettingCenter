     ...
     // 标志变量，用于判断两个上传是否都完成
        ...
        function save(){
            ...
            if(hasFile1 || hasFile2) {
                // 如果有文件需要上传，提示用户必须上传所有选择的文件
                if(hasFile1 && hasFile2) {
                    bootbox.alert("请确保两个文件都上传完成后才能提交表单！");
                } else if(hasFile1) {
                    bootbox.alert("请确保第一个文件上传完成后才能提交表单！");
                } else if(hasFile2) {
                    bootbox.alert("请确保第二个文件上传完成后才能提交表单！");
                }
                return false;
            }
                ...
                success: function(data){
                     if("success" == data.result){                       
                         ...
                         if(hasFile1 && hasFile2) {
                             ...
                             // 第二个文件会在第一个完成后通过onQueueComplete触发
                         } else if(hasFile1) {
                             // 只有第一个有文件
                             $('#uploadifive1').uploadifive('upload');
                         } else if(hasFile2) {
      ...
      //====================上传=================
       ...
       $("#uploadifive2").uploadifive({
           ...
           "onUploadComplete": function (file, data) {
               var str2 = data.trim();
               $("#attach-customer-approval").val(str2);
               upload2Done = true;
               checkUploadsComplete();
           },
           "onQueueComplete": function (uploads) {
           //全部上传完毕执行
               // 这里不需要提交表单，因为checkUploadsComplete会处理
           },
           "onSelect": function (queue) {
               $("#hasTp2").val("ok");
           }
       ...
       function checkUploadsComplete() {
           ...
           // 检查是否有文件需要上传但尚未完成
           if ((hasFile1 && !upload1Done) || (hasFile2 && !upload2Done)) {
               // 如果有文件需要上传但尚未完成，给出提示
               if (hasFile1 && !upload1Done && hasFile2 && !upload2Done) {
                   bootbox.alert("请等待两个文件都上传完成后再提交！");
               } else if (hasFile1 && !upload1Done) {
                   bootbox.alert("请等待第一个文件上传完成后再提交！");
               } else if (hasFile2 && !upload2Done) {
                   bootbox.alert("请等待第二个文件上传完成后再提交！");
               }
               return false;
           }
           
           // 如果所有选择的文件都已上传完成，提交表单
           $("#Form").submit();
           $("#zhongxin").hide();
           $("#zhongxin2").show();
           return true;
