...
public class McncheckTSMCController extends BaseController{
	String menuUrl = "mcncheck/tsmc.do";  // 菜单地址(权限用)
	@Resource(name="fhlogService")
	private FHlogManager FHLOG;
	@Resource(name="McncheckTSMCService")
	private  McncheckManager mcncheckTSMCService;
	...
	public ModelAndView listCP(Page page) throws Exception{	
		...
		if("2".equals(type)){
			...
			List<PageData> varList = mcncheckTSMCService.list(page); 
		...
		if("2".equals(type)){
			mv.setViewName("mcncheck/tsmc/tsmc_list_r");
		}else{
			mv.setViewName("mcncheck/tsmc/tsmc_list");
		}
		return mv;	
	}
	...
	public void delete(PrintWriter out) throws Exception{
		...
		pd = mcncheckTSMCService.findById(pd);
		//开启审核 删除保留
		mcncheckTSMCService.delete(pd);
		//添加进审批表			
		approvalService.save(Jurisdiction.getUsername(),"删除",JSON.toJSONString(pd),"MCN_TSMC",Jurisdiction.getRoleParentID());
		
		//发送邮件
		sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "删除", pd, "MCN_TSMC_A");
	...
	public ModelAndView goAdd()throws Exception{	
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("mcncheck/tsmc/tsmc_edit");
	 ...
	 /**保存修改
		...
		public ModelAndView save()throws Exception{
			...
			//增加审批功能 此处先注释
			//mcncheckTSMCService.save(pd); 
			//添加进审批表			
			approvalService.save(Jurisdiction.getUsername(),"新增",JSON.toJSONString(pd),"MCN_TSMC",Jurisdiction.getRoleParentID());
			
			//发送邮件
			sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "新增", pd, "MCN_TSMC_A");
		...
		public ModelAndView edit()throws Exception{
			...
			//mcncheckTSMCService.edit(pd);	//执行修改
			mcncheckTSMCService.delete(pd);
			//添加进审批表	
			approvalService.save(Jurisdiction.getUsername(),"修改",JSON.toJSONString(pd),"MCN_TSMC",Jurisdiction.getRoleParentID());
			//发送邮件
			sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "修改", pd, "MCN_TSMC_A");
		...
		public void agree(PrintWriter out) throws Exception{
			...
			if("新增".equals(pd.get("STATUS"))){
				pd.put("newZT", "1");
				mcncheckTSMCService.save(pdd); 	//将新增mcn信息保存进去
				approvalService.edit(pd);		//修改状态为审核同意
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "新增", pdd, "MCN_TSMC_T");
			}else if("删除".equals(pd.get("STATUS"))){
				pd.put("newZT", "1");
				approvalService.edit(pd);		//修改状态为审核同意
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "删除", pdd, "MCN_TSMC_T");
			}else if("修改".equals(pd.get("STATUS"))){
				pd.put("newZT", "1");
				approvalService.edit(pd);		//修改状态为审核同意
				mcncheckTSMCService.save(pdd);	    //修改的信息同意
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "修改", pdd, "MCN_TSMC_T");
			}
		...
		public void deny(PrintWriter out) throws Exception{
			...
			if("新增".equals(pd.get("STATUS"))){
				...
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "新增", pdd, "MCN_TSMC_F");
			}else if("删除".equals(pd.get("STATUS"))){
				pd.put("newZT", "2");
				approvalService.edit(pd);		//修改状态为审核不同意
				mcncheckTSMCService.save(pdd);	//删除的信息还原
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "删除", pdd, "MCN_TSMC_F");
			}else if("修改".equals(pd.get("STATUS"))){
				...
				pd.put("newZT", "2");
				approvalService.edit(pd);		//修改状态为审核同意				
				mcncheckTSMCService.save(pdb);	//修改的信息还原
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "修改", pdd, "MCN_TSMC_F");
			}
		...
		public Object hasU(){
			...
			try{
				pd = this.getPageData();
				int count=mcncheckTSMCService.findBySimilar(pd);
				if( count>0){
					errInfo = "error";
				}
			} catch(Exception e){
