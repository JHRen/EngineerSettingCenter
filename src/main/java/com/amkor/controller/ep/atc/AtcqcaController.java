...
import com.amkor.service.ep.atc.impl.AtcqcaService;
...
public class AtcqcaController extends BaseController{
	String menuUrl = "atc/qca/list.do";  // 菜单地址(权限用)
	@Resource(name="atcqcaService")
	private  AtcqcaService atcqcaService;
	...
	public ModelAndView listQCA(Page page) throws Exception{	
		...
		//审核视图
		if("2".equals(type)){
			pd.put("TABLE_NAME", "lodfile_atcqca");
			...
			List<PageData> varList = atcqcaService.list(page); 
		...
		if("2".equals(type)){
			mv.setViewName("lodfile/atc_qca/atcqca_list_r");
		}else{
			mv.setViewName("lodfile/atc_qca/atcqca_list");
		}
		return mv;	
	}
	...
	public void delete(PrintWriter out) throws Exception{
		...
		logBefore(logger, Jurisdiction.getUsername()+"删除ATCQCA");
		...
		pd = atcqcaService.findById(pd);	
		
		atcqcaService.delete(pd);
		//添加进审批表			
		approvalService.save(Jurisdiction.getUsername(),"删除",JSON.toJSONString(pd),"lodfile_atcqca",Jurisdiction.getRoleParentID());
		//发送邮件
		sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "删除", pd, "lodfile_atcqca_A");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("lodfile/atc_qca/atcqca_edit");
	 ...
	 /**保存修改
		...
		public ModelAndView save()throws Exception{
			...
			logBefore(logger, Jurisdiction.getUsername()+"新增ATC QCA数据");
			String cxlogid=UuidUtil.get32UUID();
			Session session = Jurisdiction.getSession();
			User user = (User)session.getAttribute(Const.SESSION_USER);	
			...
			//atcqcaService.save(pd); 
			//添加进审批表			
			approvalService.save(Jurisdiction.getUsername(),"新增",JSON.toJSONString(pd),"lodfile_atcqca",Jurisdiction.getRoleParentID());
			//发送邮件
			sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "新增", pd, "lodfile_atcqca_A");
		...
		public ModelAndView edit()throws Exception{
			logBefore(logger, Jurisdiction.getUsername()+"  update ATC QCA");
			String cxlogid=UuidUtil.get32UUID();
			Session session = Jurisdiction.getSession();
			User user = (User)session.getAttribute(Const.SESSION_USER);
			...
			//atcqcaService.editQCA(pd);	//执行修改
			atcqcaService.delete(pd);
			approvalService.save(Jurisdiction.getUsername(),"修改",JSON.toJSONString(pd),"lodfile_atcqca",Jurisdiction.getRoleParentID());
			//发送邮件
			sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "修改", pd, "lodfile_atcqca_A");
		...
		public void agree(PrintWriter out) throws Exception{
			...
			if("新增".equals(pd.get("STATUS"))){
				pd.put("newZT", "1");
				atcqcaService.save(pdd); 	//将新增信息保存进去
				approvalService.edit(pd);		//修改状态为审核同意
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "新增", pdd, "lodfile_atcqca_T");
			}else if("删除".equals(pd.get("STATUS"))){
				pd.put("newZT", "1");
				approvalService.edit(pd);		//修改状态为审核同意
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "删除", pdd, "lodfile_atcqca_T");
			}else if("修改".equals(pd.get("STATUS"))){
				pd.put("newZT", "1");
				approvalService.edit(pd);		//修改状态为审核同意
				atcqcaService.save(pdd);	    //修改的信息同意
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "修改", pdd, "lodfile_atcqca_T");
			}
		...
		public void deny(PrintWriter out) throws Exception{
			...
			if("新增".equals(pd.get("STATUS"))){
				...
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "新增", pdd, "lodfile_atcqca_F");
			}else if("删除".equals(pd.get("STATUS"))){
				pd.put("newZT", "2");
				approvalService.edit(pd);		//修改状态为审核不同意
				atcqcaService.save(pdd);	//删除的信息还原
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "删除", pdd, "lodfile_atcqca_F");
			}else if("修改".equals(pd.get("STATUS"))){
				...
				pd.put("newZT", "2");
				approvalService.edit(pd);		//修改状态为审核同意				
				atcqcaService.save(pdb);	//修改的信息还原
				//发送邮件
				sendmailService.sendToManager(Jurisdiction.getUsername(),Jurisdiction.getRecipient(), "修改", pdd, "lodfile_atcqca_F");
			}
		...
		public Object hasU(){
			...
			try{
				pd = this.getPageData();
				int count=atcqcaService.findBySimilar(pd);
				if( count>0){
					errInfo = "error";
				}
			} catch(Exception e){
