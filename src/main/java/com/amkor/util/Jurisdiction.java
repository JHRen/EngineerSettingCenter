...
public class Jurisdiction {
		...
		for(int i=0;i<menuList.size();i++){
			if(menuList.get(i).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
				if(!menuList.get(i).isHasMenu()){				//判断有无此菜单权限
					...
					Boolean isAdmin = "40607".equals(USERNAME);
					Boolean badd = false;
					Boolean bdel = false;
					Boolean bedit = false;
					Boolean bcha = false;
					...
					if(isAdmin){
						...
						bapr = RightsHelper.testRights(map.get("aprs"), MENU_ID);
						/**读取副职角色按钮权限**/
						if(!badd){
						...
						if(!bapr){
							List<String> aprsList = maps.get("aprsList");
							if(null != aprsList){
								for(int n=0;n<aprsList.size();n++){
									bapr = RightsHelper.testRights(aprsList.get(n), MENU_ID);
									if(bapr) break;
								}
							}	
						}
	 ...
	 * 按钮权限(方法中校验)
	 * @param menuUrl  菜单路径
	 * @param type  类型(add、del、edit、cha、apr)
	 * @return
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * @return
	 */
	public static String getRoleParentID(){
		return getSession().getAttribute(Const.SESSION_ROLE_PARENTID).toString();
	}
	/**获取邮箱列表
	 * @return
	 * @throws IOException 
	 */
	@SuppressWarnings("unchecked")
	public static List<String> getRecipient() throws IOException{
		return (List<String>) getSession().getAttribute(Const.EMAILRECIPIENT);
	}
	/**获取当前登录的用户姓名
	 * @return
	 */
