			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="sockettdlimit/list.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' style="vertical-align: middle;">${var.SOCKET_ID}</td>
											...
											<td class='center' style="vertical-align: middle;">${var.TD_LIMIT}</td>
											...
											<td class="center" style="width:86px"> 
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.ID}','${var.SOCKET_ID }');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.ID}','${var.SOCKET_ID} ');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								...
								<c:if test="${QX.apr == 1 }">
										<a class="btn btn-mini btn-warning" onclick="window.location.href='<%=basePath%>sockettdlimit/list.do?type=2';">切换审批视图</a>
								</c:if>
							</td>
...
<%@ include file="../../system/index/foot.jsp"%>
	...
	<script type="text/javascript">
		...
		//删除
		function del(ID,SOCKET_ID){
			bootbox.confirm("确定要删除SOCKET_ID为["+SOCKET_ID+"]的信息吗?", function(result) {
				if(result) {
					top.jzts();            
					var url = "<%=basePath%>sockettdlimit/delete.do?ID="+ID+"&tm="+new Date().getTime();
	...
	function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>sockettdlimit/add.do';
	 diag.Width = 469;
	 diag.Height = 350;
	...
	function edit(ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="修改";
		 diag.URL = '<%=basePath%>sockettdlimit/goEdit.do?ID='+ID;
		 diag.Width = 469;
		 diag.Height = 435;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function toExcel(){
		...
		window.location.href='<%=basePath%>sockettdlimit/excel.do?keywords='+keywords;
	}
	...
	function fromExcel(){
		 ...
		 diag.URL = '<%=basePath%>sockettdlimit/goUploadExcel.do';
		 diag.Width = 300;
		 diag.Height = 150;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		 ...
		 diag.show();
			...
			function editMember(USER_PHONE){
				 ...
				 diag.Drag=true;
				 diag.Title ="资料";
				 diag.URL = '<%=basePath%>sockettdlimit/showDetails.do?USER_PHONE='+USER_PHONE;
				 diag.Width = 469;
				 diag.Height = 510;
				 diag.CancelEvent = function(){ //关闭事件
					 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
