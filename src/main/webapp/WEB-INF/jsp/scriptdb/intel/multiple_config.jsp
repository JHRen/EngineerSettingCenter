			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="intel/multiple/list.do" method="post" name="Form" id="Form">
				...
				<td>
					<div class="nav-search">
						<span class="input-icon"> <input
							...
							value="${pd.keywords}" placeholder="这里输入关键字" /> <i
	...
	<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
		<thead>
			<tr>
				...
				<th class="center" >FilePath</th>
				<th class="center" >Keywords</th>
				<th class="center" >LimtCount</th>
				...
				<c:forEach items="${varList}" var="var" varStatus="vs">
					<tr>
						...
						<td class='center' style="vertical-align: middle;"> 
					    	<c:if test="${var.FILE_PATH == '1'}">\\c3nas15\Production_unix\126\program </c:if>
					       	<c:if test="${var.FILE_PATH == '2'}">\\c3nas15\Production_win\126\Program </c:if>
						    <c:if test="${var.FILE_PATH == '3'}">\\c3nas15\Production_win\126\Burn_Program </c:if>
						</td>
					    <td class='center' style="vertical-align: middle;">${var.FILE_KEYWORDS}</td>
					    <td class='center' style="vertical-align: middle;">${var.FILE_LIMIT}</td>
...
<%@ include file="../../system/index/foot.jsp"%>
	...
	<script type="text/javascript">
		...
		function del(ID,MCN_Name){
			bootbox.confirm("确定要删除当前信息吗?", function(result) {
				if(result) {
					top.jzts();            
					var url = "<%=basePath%>intel/multiple/delete.do?ID="+ID+"&tm="+new Date().getTime();
	...
	function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>intel/multiple/add.do';
	 diag.Width = 469;
	 diag.Height = 252;
	...
	function edit(ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="修改";
		 diag.URL = '<%=basePath%>intel/multiple/goEdit.do?ID='+ID;
		 diag.Width = 469;
		 diag.Height = 252;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
