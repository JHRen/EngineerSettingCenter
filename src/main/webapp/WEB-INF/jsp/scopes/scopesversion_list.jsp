			...
			<td>
				<div class="nav-search">
					<span class="input-icon"> <input
						...
						value="${pd.keywords}" placeholder="输入hostname或CustomerCode查询" /> <i
	...
	<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center"><span data-toggle="tooltip" title="Hostname">Hostname</span></th>
									<th class="center"><span data-toggle="tooltip" title="CustomerCode">CustomerCode</span></th>
									<th class="center"><span data-toggle="tooltip" title="Factory">Factory</span></th>
									<th class="center"><span data-toggle="tooltip" title="Process">Process</span></th>
									<th class="center"><span data-toggle="tooltip" title="Platform">Platform</span></th>
									<!-- <th class="center"><span data-toggle="tooltip" title="Devicename">Devicename</span></th> -->
									<th class="center"><span data-toggle="tooltip" title="VersionName">VersionName</span></th>
									<th class="center"><span data-toggle="tooltip" title="UserName">UserName</span></th>
                                ...
                                <c:when test="${not empty varList}">
                                    ...
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr>
                                            ...
                                            <td class='center' style="vertical-align: middle;">${var.Hostname}</td>
                                            <td class='center' style="vertical-align: middle;">${var.CustomerCode}</td>
                                            <td class='center' style="vertical-align: middle;">${var.Factory}</td>
                                            <td class='center' style="vertical-align: middle;">${var.Process}</td>
                                            <td class='center' style="vertical-align: middle;">${var.Platform}</td>
                                            <%-- <td class='center' style="vertical-align: middle;">${var.Devicename}</td>  --%> 
                                            <td class='center' style="vertical-align: middle;">${var.VersionName}</td>      
...
function del(ID){
	bootbox.confirm("确定要删除选中的"+ID+"数据吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>scopes/delete.do?ID="+ID+"&tm="+new Date().getTime();
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>scopes/goAdd.do';
	 diag.Width = 469;
	 diag.Height = 540;
...
function edit(ID){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>scopes/goEdit.do?ID='+ID;
	 diag.Width = 469;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
