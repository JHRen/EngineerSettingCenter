			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="mcncheck/gf/list.do?type=2" method="post" name="Form" id="Form">
	...
	<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" ><span data-toggle="tooltip" title="MCN Name">MCN Name</span></th>
									<th class="center" ><span data-toggle="tooltip" title="GF Part">GF Part</span></th>
									<th class="center"><span data-toggle="tooltip" title="Test PGM1">Test PGM1</span></th>
									<th class="center"><span data-toggle="tooltip" title="Job Name">Job Name</span></th>
								...
								<c:when test="${not empty varList_r}">
									...
									<c:forEach items="${varList_r}" var="var" varStatus="vs">
										...
										<tr>
											...
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].MCN_Name}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].GF_Part}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].TEST_PGM1}</td>
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].JOB_NAME}</td>												
											<!-- 操作按钮start -->
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.apr == 1 }">
										<a class="btn btn-mini btn-warning" onclick="window.location.href='<%=basePath%>mcncheck/gf/list.do';">切换回数据视图</a>
								</c:if>
							</td>
	...
	<script type="text/javascript">
		...
		function agree(ID){
			bootbox.confirm("确定要审批通过吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>mcncheck/gf/agree.do?approvalID="+ID+"&tm="+new Date().getTime();
		...
		function deny(ID){
			bootbox.confirm("确定不同意审批吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>mcncheck/gf/deny.do?approvalID="+ID+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage('${page.currentPage}');
					});
				};
