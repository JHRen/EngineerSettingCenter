			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="mcncheck/samsung/list.do?type=2" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList_r}">
									...
									<c:forEach items="${varList_r}" var="var" varStatus="vs">
										...
										<tr>
											...
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].TargetDevice}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Nickname}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].QCTname}</td>
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].MCNDevice}</td>												
											<!-- 操作按钮start -->
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.apr == 1 }">
										<a class="btn btn-mini btn-warning" onclick="window.location.href='<%=basePath%>mcncheck/samsung/list.do';">切换回数据视图</a>
								</c:if>
							</td>
	...
	<script type="text/javascript">
		...
		function agree(ID){
			bootbox.confirm("确定要审批通过吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>mcncheck/samsung/agree.do?approvalID="+ID+"&tm="+new Date().getTime();
		...
		function deny(ID){
			bootbox.confirm("确定不同意审批吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>mcncheck/samsung/deny.do?approvalID="+ID+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage('${page.currentPage}');
					});
				};
