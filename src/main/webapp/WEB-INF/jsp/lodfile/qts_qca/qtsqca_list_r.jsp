			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="qts/qca/list.do?type=2" method="post" name="Form" id="Form">
			...
			<td>
				<div class="nav-search">
					<span class="input-icon"> <input
						...
						value="${pd.keywords}" placeholder="这里输入关键词检索" /> <i
							...
							<thead>
								<tr>
									...
									<th class="center"><span data-toggle="tooltip" title="Customer Code">Customer Code</span></th>
										<th class="center"><span data-toggle="tooltip" title="MultiJobname">MultiJobname</span></th>
								...
								<c:when test="${not empty varList_r}">
									...
									<c:forEach items="${varList_r}" var="var" varStatus="vs">
										...
										<tr>
											...
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].device}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].family_name}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].operation}</td>
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].operation_code}</td>	
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].customer_code}</td>
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].MultiJobname}</td>
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].lod_file}</td>											
											<!-- 操作按钮start -->
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.apr == 1 }">
										<a class="btn btn-mini btn-warning" onclick="window.location.href='<%=basePath%>qts/qca/list.do';">切换回数据视图</a>
								</c:if>
							</td>
	...
	<script type="text/javascript">
		...
		function agree(ID){
			bootbox.confirm("确定要审批通过吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>qts/qca/agree.do?approvalID="+ID+"&tm="+new Date().getTime();
		...
		//审批不通过
	 	function deny(ID){
			bootbox.confirm("确定不同意审批吗?", function(result) {
				if(result) {
					top.jzts(); 					
					var url = "<%=basePath%>qts/qca/deny.do?approvalID="+ID+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage('${page.currentPage}');
					});
					 $.snackbar({ content: "审批成功!", timeout: 2000 });
				};
			});
		} 
