			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="atc/cp/list.do?type=2" method="post" name="Form" id="Form">
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.apr == 1 }">
										<a class="btn btn-mini btn-warning" onclick="window.location.href='<%=basePath%>atc/cp/list.do';">切换回数据视图</a>
								</c:if>
							</td>
	...
	<script type="text/javascript">
		...
		function agree(ID){
			bootbox.confirm("确定要审批通过吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>atc/cp/agree.do?approvalID="+ID+"&tm="+new Date().getTime();
		...
		function deny(ID){
			bootbox.confirm("确定不同意审批吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>atc/cp/deny.do?approvalID="+ID+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage('${page.currentPage}');
					});
				};
