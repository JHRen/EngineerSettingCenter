...
<!-- 检索  -->
<form action="otversion/list.do" method="post" name="Form" id="Form">
			...
			<td>
				<div class="nav-search">
					<span class="input-icon"> <input
						...
						value="${pd.keywords}" placeholder="这里输入FactoyType" /> <i
	...
	<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center"><span data-toggle="tooltip" title="FactoryType">FactoryType</span></th>
									<th class="center"><span data-toggle="tooltip" title="MachineType">MachineType</span></th>
									<th class="center"><span data-toggle="tooltip" title="FactoryLocation">FactoryLocation</span></th>
									<th class="center" style="width: 100px;"><span data-toggle="tooltip" title="Path">Path</span></th>
									<th class="center" style="width: 100px;"><span data-toggle="tooltip" title="Version">Version</span></th>
									<th class="center"><span data-toggle="tooltip" title="UpdateTime">UpdateTime</span></th>
									<th class="center"><span data-toggle="tooltip" title="Result">Result</span></th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' style="vertical-align: middle;">${var.FactoryType}</td>
										    <td class='center' style="vertical-align: middle;">${var.MachineType}</td>
										    <td class='center' style="vertical-align: middle;">${var.FactoryLocation}</td>
										    <td class='center' style="width: 100px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;  vertical-align: middle;">${var.Path}</td>
											<td class='center' style=" width: 100px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; vertical-align: middle;">${var.Version}</td>												
											<td class='center' style="vertical-align: middle;">${var.UpdateTime}</td>
											<td class='center' style="vertical-align: middle;">${var.Result}</td>
...
function del(ID){
	bootbox.confirm("确定要删除选中的"+ID+"数据吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>otversion/delete.do?ID="+ID;
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>otversion/goAdd.do';
	 diag.Width = 469;
	 diag.Height = 400;
...
function edit(ID){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>otversion/goEdit.do?ID='+ID;
	 diag.Width = 469;
	 diag.Height = 400;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
