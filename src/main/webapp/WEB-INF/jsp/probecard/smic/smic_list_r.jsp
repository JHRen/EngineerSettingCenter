			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
<!-- 检索  -->
<form action="probecard/smic/list.do?type=2" method="post" name="Form" id="Form">
	...
	<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" ><span data-toggle="tooltip" title="Customer">Customer</span></th>
									...
									<th class="center"><span data-toggle="tooltip" title="Touchdown Per Wafer">Touchdown Per Wafer</span></th>
									<th class="center"><span data-toggle="tooltip" title="Touchdown Limit">Touchdown Limit</span></th>
									<th class="center"><span data-toggle="tooltip" title="Gross_Die_Per_Wafer">GrossDiePerWafer</span></th>
								...
								<c:when test="${not empty varList_r}">
									...
									<c:forEach items="${varList_r}" var="var" varStatus="vs">
										...
										<tr>
											...
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Cust_Tool_Name}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Tool_Name}</td>
										    <td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Customer}</td>
											...
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Touchdown_Per_Wafer}</td>	
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Touchdown_Limit}</td>	
											<td class='center' style="vertical-align: middle;">${mcnList[vs.count-1].Gross_Die_Per_Wafer}</td>	
											<!-- 操作按钮start -->
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.apr == 1 }">
										<a class="btn btn-mini btn-warning" onclick="window.location.href='<%=basePath%>probecard/smic/list.do';">切换回数据视图</a>
								</c:if>
							</td>
	...
	<script type="text/javascript">
		...
		function agree(ID){
			bootbox.confirm("确定要审批通过吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>probecard/smic/agree.do?approvalID="+ID+"&tm="+new Date().getTime();
		...
		function deny(ID){
			bootbox.confirm("确定不同意审批吗?", function(result) {
				if(result) {
					top.jzts();   			
					var url = "<%=basePath%>probecard/smic/deny.do?approvalID="+ID+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage('${page.currentPage}');
					});
				};
